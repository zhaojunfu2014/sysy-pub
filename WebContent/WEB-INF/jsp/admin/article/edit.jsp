...
-->
	...
	<body onload="init();">
		<sf:form id="aform" modelAttribute="article" method="post"
			action="${pageContext.request.contextPath}/admin/article/edit/${article.id}" onsubmit="return checkDate();">
			<div style="background-color: #353C44" width="100%"
				style="padding-top:10px">
				<span class="STYLE1">编辑文章</span>
			</div>
			<table width="100%">
				<tr>
					...
					<td>
						<select id="menuId" name="menuId">
							<c:forEach items="${menus}" var="m">
								<c:if test="${m.id==article.menu.id}">
								...
								<c:forEach items="${m.childs}" var="mc">
									<c:if test="${mc.id==article.menu.id}">
				...
				<tr>
					...
					<td>
						<textarea name="content" id="content"
							style="width: 750px; height: 300px; visibility: hidden;">${article.content }</textarea>
				...
				<tr>
					...
					<td>
						<input type="text" id="pdate" name="pdate" value="${article.pubDateString }">
